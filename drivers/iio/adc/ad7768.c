...
#define map_power_mode_to_regval(x)		((x > 0) ? x + 1 : x)
#define map_regval_to_power_mode(x)		((x > 0) ? x - 1 : x)
				 ...
				 unsigned int mode)
{
	...
	regval = map_power_mode_to_regval(mode);
				 ...
				 const struct iio_chan_spec *chan)
{
	...
	st->power_mode = map_regval_to_power_mode(
			 AD7768_POWER_MODE_GET_POWER_MODE(regval));
...
static void ad7768_set_available_sampl_freq(struct ad7768_state *st)
{
	...
	for (mode = 0; mode < AD7768_NUM_POWER_MODES; mode++) {
		avail_freq = &st->avail_freq[mode];
		for (dec = ARRAY_SIZE(ad7768_dec_rate) - 1; dec >= 0;  dec--) {
			struct ad7768_freq_config freq_cfg;

			freq_cfg.dec_rate = dec;
			freq_cfg.freq = mclk / (ad7768_dec_rate[dec] *
